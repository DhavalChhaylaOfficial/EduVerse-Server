...
exports.getAllCategoryCourses = async (req, res, next) => {
  try {
    ...
    if (categoryId) {
      ...
      if (!reqCat) {
        return next(new ErrorResponse(`Category with ID ${categoryId} not found`, 404));
      }
    ...
    const otherCategoryCourses = await Category.findById(categoriesExceptRequested[getRandomInt(categoriesExceptRequested.length)]._id)
      .populate({
        path: 'courses',
        match: { status: 'Published' },
        populate: { path: 'instructor' },
    ...
    const topSellingCourses = await Course.find({ status: 'Published' })
      .sort({ numberOfEnrolledStudents: 'desc' })
      .populate({
        path: 'category',
        match: { status: 'Published' },
        select: 'name',
    ...
    next(new ErrorResponse('Failed to fetch all category courses. Please try again', 500));
  }
};
